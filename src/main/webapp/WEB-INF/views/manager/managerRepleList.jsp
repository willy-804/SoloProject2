...
<link rel="stylesheet"
	href="../../../resources/CSS/managerRepleListCSS.css?version=1.6">
	...
	<div class="container">
		...
		<div class="page">
				...
				<c:if test="${paging.prev}">
					<a class="pagemove"
						href="/manager/managerRepleList?id=${paging.cri.id}&type=${paging.cri.type}&keyword=${paging.cri.keyword}&pageNum=${paging.startPage-1}&amount=${paging.cri.amount}"><</a>
				</c:if>
				...
				<c:forEach begin="${paging.startPage}" end="${paging.endPage}"
					...
					<c:choose>
						<c:when test="${paging.cri.pageNum==num}">
							<a class="selectpage"
								href="/manager/managerRepleList?id=${paging.cri.id}&type=${paging.cri.type}&keyword=${paging.cri.keyword}&pageNum=${num}&amount=${paging.cri.amount}">${num}</a>
						</c:when>
						<c:otherwise>
				...
				<c:if test="${paging.next}">
					<a class="pagemove"
						href="/manager/managerRepleList?id=${paging.cri.id}&type=${paging.cri.type}&keyword=${paging.cri.keyword}&pageNum=${paging.endPage+1}&amount=${paging.cri.amount}">></a>
				</c:if>
			</div>
			<!-- 페이징 -->
