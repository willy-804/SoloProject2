...
function main(){
    setMonthTitle(year,mon);
    makeCalendar(year,mon,getDayOfMon(mon,year));
    todoTitle.textContent = `DO NOT FORGET ${year} - ${mon} - ${currentDay}`;
    displayToDoOnDays();
}
...
Day.addEventListener('click',(event)=>{
    ...
    if(event.target.className!=='disabled'){
        clearEvent();
        todoTitle.textContent = `DO NOT FORGET  ${year} - ${mon} - ${event.target.textContent}`;
...
reset.addEventListener('click',()=>{
    const result = prompt(`${year} - ${mon} - ${DayOfChoice} 의 일정을 삭제하려면 Y 아니면 N 을 입력하세요`);
    const YMD = year+'-'+mon+'-'+DayOfChoice;
    if(result==='y'){
...
allReset.addEventListener('click',()=>{
    const result = prompt(`모든 일정을 삭제하려면 Y 아니면 N을 입력하세요`);
    if(result==='y'){
        localStorage.clear();
        displayToDoOnDays();
    }
